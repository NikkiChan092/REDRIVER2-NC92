...
void SetCarReflection(int enabled, plotCarGlobals* pg, int otOfs)
{
	...
	if (enabled)
		SetPsyXTexture(tex, gCarReflectionTexture, 480, 480);
	else
	...
	while (numTris > 0)
	{
		...
		if (Z > -1)
		{
			...
			*(uint*)&prim->r0 = 0x202020;
			...
			prim->u0 = 320 + (FIXEDH(v0->vx * cs) + FIXEDH(v0->vz * sn) >> 3) - (pg->shineyX & 63);
			prim->u1 = 320 + (FIXEDH(v1->vx * cs) + FIXEDH(v1->vz * sn) >> 3) - (pg->shineyX & 63);
			prim->u2 = 320 + (FIXEDH(v2->vx * cs) + FIXEDH(v2->vz * sn) >> 3) - (pg->shineyX & 63);

			prim->v0 = 320 + (FIXEDH(v0->vx * sn) - FIXEDH(v0->vz * cs) >> 3) + (pg->shineyY & 63);
			prim->v1 = 320 + (FIXEDH(v1->vx * sn) - FIXEDH(v1->vz * cs) >> 3) + (pg->shineyY & 63);
			prim->v2 = 320 + (FIXEDH(v2->vx * sn) - FIXEDH(v2->vz * cs) >> 3) + (pg->shineyY & 63);
